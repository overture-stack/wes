...
import bio.overture.wes.model.api.RunId;
import bio.overture.wes.model.api.RunListResponse;
import bio.overture.wes.model.api.RunLog;
import bio.overture.wes.model.api.RunStatus;
import bio.overture.wes.service.RunService;
...
public class RunsApiController implements RunsApi {
  ...
  public RunsApiController(
      RunService runService, ObjectMapper objectMapper, HttpServletRequest request) {
    this.runService = runService;
    this.objectMapper = objectMapper;
    this.request = request;
  }

  public ResponseEntity<RunId> cancelRun(
      @ApiParam(value = "", required = true) @PathVariable("run_id") String runId) {
    String accept = request.getHeader("Accept");
    if (accept != null && accept.contains("application/json")) {
      try {
        return new ResponseEntity<RunId>(
            objectMapper.readValue("{  \"run_id\" : \"run_id\"}", RunId.class),
  ...
  public ResponseEntity<RunLog> getRunLog(
    ...
    if (accept != null && accept.contains("application/json")) {
      try {
        return new ResponseEntity<RunLog>(
  ...
  public ResponseEntity<RunStatus> getRunStatus(
    ...
    if (accept != null && accept.contains("application/json")) {
      try {
        return new ResponseEntity<RunStatus>(
            objectMapper.readValue(
  ...
  public ResponseEntity<RunListResponse> listRuns(
    ...
    if (accept != null && accept.contains("application/json")) {
      try {
        return new ResponseEntity<RunListResponse>(
            objectMapper.readValue(
  ...
  public ResponseEntity<RunId> runWorkflow(
      @ApiParam(value = "") @RequestParam(value = "workflow_params", required = false)
          String workflowParams,
      @ApiParam(value = "") @RequestParam(value = "workflow_type", required = false)
          String workflowType,
      @ApiParam(value = "") @RequestParam(value = "workflow_type_version", required = false)
          String workflowTypeVersion,
      @ApiParam(value = "") @RequestParam(value = "tags", required = false) String tags,
      @ApiParam(value = "") @RequestParam(value = "workflow_engine_parameters", required = false)
          String workflowEngineParameters,
      @ApiParam(value = "") @RequestParam(value = "workflow_url", required = false)
          String workflowUrl,
      @ApiParam(value = "") @RequestParam(value = "workflow_attachment", required = false)
          List<byte[]> workflowAttachment) {
    val id = runService.create().toString();
    return new ResponseEntity<RunId>(new RunId().runId(id), HttpStatus.OK);
  }
}
